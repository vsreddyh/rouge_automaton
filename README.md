...
- Mention-gated replies (`require_mention: true`), `auto_thread: true`: a mention opens a thread that stays a live session — follow-ups in the thread answer without a fresh mention (same relay session, keyed by thread ID). Outside threads, a mention is required; configured free-response channels answer without a mention (each costs a model call — keep the list short).
- Greetings (`hi`/`hello`/`hey`, also `yo`/`o7`): answered by the model as one flat line, e.g. `Helldiver. Make it quick.` — no helpdesk intro, no capability list.
